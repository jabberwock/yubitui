...
2. Run `collab list` and READ the output before doing anything else Î“Ã‡Ã¶ treat pending messages as blocking
...
4. Run `collab watch --role "<project>: <your current task>"` with real context, not a leftover or generic description
   Example: `collab watch --role "<my-instance-id>: phase 09 OathScreen widget implementation"`
...
- A public API changed: trait signature, method rename, new required field
  Example: `collab add @<other-worker> "renamed Widget::render to Widget::draw in widget/mod.rs Î“Ã‡Ã¶ update any impl blocks"`
...
Ensure you are using Pilot and/or tmux for testing the UI. collab any issues you're having.